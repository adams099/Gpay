...
class VA_transaction_unpaid extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('va_transaction_history_model');
	...
	public function index()
	{
		...
		$receiving_inst_id = $this->session->userdata('receiving_inst_id');
		// Clear search session lama
		$this->clear_search_session();
		
		// Total rows
		$data['total_rows'] = $this->va_transaction_history_model->get_total_row_trans_hist_unpaid($merchant_id, $receiving_inst_id, $merchant_dashboard_id);
		...
		$data['trans_hist'] =  $this->va_transaction_history_model->get_va_history_unpaid_only($merchant_id, $receiving_inst_id, $config['per_page'], $offset, $merchant_dashboard_id);
		$data['store'] =  $this->va_transaction_history_model->get_store_by_merchant_id($merchant_id);
        ...
        //------------------------------ TEMPLATING ------------------------------
		$template['content'] = $this->load->view('va_transaction_unpaid/index', $data, true);
	...
	public function detail(){
		return $this->va_transaction_history_model->get_detil_va_history("3978900000000003");
	...
	private function clear_search_session()
	{
        ...
        $this->session->set_userdata('filter_trans_unpaid', $search_param);
	}
	...
	public function submit_search()
    {
		...
		$search_param = array(
			...
			'payment_failed' => $this->input->post('payment_failed'),
		...
		redirect('va_transaction_unpaid/search');
	
	}

	public function search()
	{
		$search_param = $this->session->userdata('filter_trans_unpaid');
		$merchant_id = $this->session->userdata('merchant_qris_code');
		$receiving_inst_id = $this->session->userdata('receiving_inst_id');
		$merchant_dashboard_id = $this->session->userdata('id');
		
		// Total rows
		$data['total_rows'] = $this->va_transaction_history_model->get_total_row_trans_hist_unpaid_filtered($search_param, $merchant_id, $receiving_inst_id, $merchant_dashboard_id);
		...
		$data['trans_hist'] =  $this->va_transaction_history_model->get_va_history_unpaid_filtered($search_param, $merchant_id, $receiving_inst_id, $config['per_page'], $offset, $merchant_dashboard_id );
		$data['store'] =  $this->va_transaction_history_model->get_store_by_merchant_id($merchant_id);
		// $data['offset'] = "Showing ".strval($showing_data_count)." Of ".strval($data['total_rows']);
		$data['offset'] = "Showing ".strval($data_count_begin)." to ".strval($data_count_last)." out of ".$data['total_rows'];
	...
	private function pagination_config($total_rows)
	{
		...
		$config['base_url'] = site_url('va_transaction_unpaid/index');
	...
	{
		$config = $this->global_library->global_pagination_config();
		$config['base_url'] = site_url('va_transaction_unpaid/search');
		$config['total_rows'] = $total_rows;
	...
	public function export(){
		...
		$i=2;
		$data=  $this->va_transaction_history_model->get_va_history_export($search_param, $merchant_id, $receiving_inst_id, $merchant_dashboard_id);
	...
	public function export_csv(){
		...
		// $data=  $this->va_transaction_history_model->get_va_history();
		...
		$filename = 'Virtual Account Transaction-History-'.date("Ymd_His").".csv";
		...
		foreach($data as $row) {
			$lineData = array($row['order_number'], $row['cust_phone'], $row['va_number'], $row['customer_name'], $row['cust_email'], date_format(date_create($row['time_trx']), 'd/m/Y H:i:s'), str_replace(',','',$row['amount']), $row['bca_payment_channel'],'BCA', $row['response_message']);
	...
	public function updateFlag(){
		...
		foreach ($orders as $row){
			$this->va_transaction_history_model->update_flag_unpaid($row);
		}	
